...
b8 filesystem_read_line(file_handle *handle, char **line_buf) {
	if (!handle->is_valid || !handle->handle) { return false; }

	char buffer[32000];
	if (fgets(buffer, 32000, (FILE *)handle->handle) == 0) { return false; }
...
b8 filesystem_read(file_handle *handle, u64 data_size, void *out_data, u64 *out_bytes_read) {
	if (!handle->is_valid || !handle->handle || !out_data) { return false; }
...
b8 filesystem_write(file_handle *handle, u64 data_size, void const *data, u64 *out_bytes_written) {
	if (!handle->is_valid || !handle->handle) { return false; }
