...
b8 platform_system_startup(u64 *memory_requirement,
	...
	*memory_requirement = sizeof(platform_state);
	...
	xcb_change_property(state_ptr->connection,
						...
						(u32)strlen(application_name),
						application_name);

	// Tell the server to notify when the window manager attempts to destroy the
	// window.
	xcb_intern_atom_cookie_t wm_delete_cookie =
		xcb_intern_atom(state_ptr->connection, 0, strlen("WM_DELETE_WINDOW"), "WM_DELETE_WINDOW");
	xcb_intern_atom_cookie_t wm_protocols_cookie =
		xcb_intern_atom(state_ptr->connection, 0, strlen("WM_PROTOCOLS"), "WM_PROTOCOLS");
	...
	if (stream_result <= 0) {
		SFATAL("An error occurred when flushing the stream: %d", stream_result);
		return false;
	}
...
b8 platform_pump_messages() {
	...
	while (true) {
		xcb_generic_event_t *event = xcb_poll_for_event(state_ptr->connection);
		if (event == 0) { break; }
		...
		switch (event->response_type & ~0x80) {
			...
			case XCB_KEY_RELEASE: {
				...
				KeySym key_sym     = XkbKeycodeToKeysym(state_ptr->display, (KeyCode)code, 0, code & ShiftMask ? 1 : 0);
...
static void print_with_colour(const char *message, u8 colour, FILE *stream) {
	// FATAL, ERROR, WARN, INFO, DEBUG, TRACE
	const char *colour_strings[] = {"0;41", "1;31", "1;33", "1;32", "1;34", "1;36"};
	fprintf(stream, "\033[%sm%s\033[0m\n", colour_strings[colour], message);
}

void platform_console_write(const char *message, u8 colour) { print_with_colour(message, colour, stdout); }

void platform_console_write_error(const char *message, u8 colour) { print_with_colour(message, colour, stderr); }
