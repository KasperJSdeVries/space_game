...
void vulkan_command_buffer_begin(vulkan_command_buffer *command_buffer,
	...
	if (is_single_use) { flags |= VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT; }
	if (is_render_pass_continue) { flags |= VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT; }
	if (is_simultaneous_use) { flags |= VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT; }
