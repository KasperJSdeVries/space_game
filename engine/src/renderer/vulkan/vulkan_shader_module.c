...
b8 shader_module_create(vulkan_context *context,
						char const *name,
						char const *type_str,
...
void shader_module_destroy(vulkan_context *context, u32 stage_index, vulkan_shader_stage *shader_stages) {
	vkDestroyShaderModule(context->device.logical_device, shader_stages[stage_index].handle, context->allocator);
}
